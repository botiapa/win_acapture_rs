...
use activation_params::SafeActivationParams;
use com::com_initialized;
use hound::WavWriter;
use notifications::{AudioSessionEventArgs, Notifications};
use processes::ProcessesManager;
use windows::{
    core::{IUnknown, Interface, GUID, HRESULT},
    Win32::{
        self,
        Foundation::{self, HANDLE, S_OK, WAIT_FAILED, WAIT_OBJECT_0, WIN32_ERROR},
        Media::Audio::*,
        System::{
...
fn main() {
    ...
    let mut pm = ProcessesManager::new().unwrap();
    let sessions = pm.query_sessions().unwrap();

    let mut nm = Notifications::new();
    let active_discord_sessions: Vec<&processes::Session> = sessions
        .iter()
        .filter(|s| s.get_process_name().as_ref().unwrap().contains("Discord.exe") && s.get_state().unwrap() == AudioSessionStateActive)
        .collect();

    let (sender, receiver) = mpsc::channel();

    cap.start_recording(
        active_discord_sessions[0].get_pid().clone(),
        move |data| {
    ...
    .unwrap();
    for i in 0..out_buffer.len() / 4 {
        let left_sample = i16::from_ne_bytes([out_buffer[i * 4], out_buffer[i * 4 + 1]]);
        let right_sample = i16::from_ne_bytes([out_buffer[i * 4 + 2], out_buffer[i * 4 + 3]]);
